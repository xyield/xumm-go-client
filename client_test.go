...
func TestXummClientCreation(t *testing.T) {
	t.Run("Default client creation with no env", func(t *testing.T) {
		c, err := NewClient()
		assert.Equal(t, &Client{BaseURL: BASEURLV1, HTTPClient: &http.Client{}}, c)
	...
	t.Run("Default client creation with only api key set", func(t *testing.T) {
		...
		c, err := NewClient()
		assert.Equal(t, &Client{BaseURL: BASEURLV1, HTTPClient: &http.Client{}, apiKey: "testApiKey"}, c)
		assert.EqualError(t, err, XUMMCREDENTIALSNOTSET)
	})
	t.Run("Default client creation with Env set", func(t *testing.T) {
		...
		assert.Equal(t, &Client{BaseURL: BASEURLV1, HTTPClient: &http.Client{}, apiKey: "testApiKey", apiSecret: "testApiSecret"}, c)
	})
	...
	t.Run("Client Creation with http client option", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, &Client{BaseURL: BASEURLV1, HTTPClient: &MockClient{}, apiKey: "testApiKey", apiSecret: "testApiSecret"}, c)
	})
}
